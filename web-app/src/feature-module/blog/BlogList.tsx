...
const BlogList = () => {
  ...
  const handleSearch = () => {
    ...
    loadBlogs(); // Reload documents with the new search term
  };
  ...
  const handleReset = () => {
    ...
    setSearchQuery("");
    setPagingParams(new PagingParams(1, pageSize));
    loadBlogs(); // Reload documents after resetting filters
  };
  ...
  const handleDelete = async () => {
    if (deleteId) {
      ...
      loadBlogs(); // Reload documents after deletion
  ...
  const handlePageSizeChange = (newPageSize: number) => {
    ...
    loadBlogs(); // Reload documents with new page size
  };
