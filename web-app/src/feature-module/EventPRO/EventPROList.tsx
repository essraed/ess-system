...
const EventPROList = () => {
  ...
  const handleSearch = () => {
    ...
    loadEventPROs(); // Reload documents with the new search term
  };
  ...
  const handleReset = () => {
    ...
    setSearchQuery("");
    setPagingParams(new PagingParams(1, pageSize));
    loadEventPROs(); // Reload documents after resetting filters
  };
  ...
  const handlePageSizeChange = (newPageSize: number) => {
    ...
    loadEventPROs(); // Reload documents with new page size
  };
  ...
  }, [userStore.token, loadEventPROs]);

  if (!eventPRO) return <LoadingSpinner />;
...
export default observer(EventPROList);
